...
def bwk(k, a, b):
    return KroneckerDelta(k + a, b + 1) * factorial2(2 * b + 1) / (factorial2(2 * k + 1) * factorial2(2 * a + 1)) * (
            2 * a + 1)
...
def cmv(k, a, b):
    p1 = 0
    if b - k + 1 >= 0:
        p1 = factorial(2 * a + 2 * b - 2 * k + 3) / (factorial(2 * a + 1) * factorial(2 * b - 2 * k + 2)) * zeta(
            2 * a + 2 * b - 2 * k + 4) / s ** (2 * a + 2 * b - 2 * k + 4)

    p2 = 0
    if a - k + 1 >= 0:
        p2 = factorial(2 * a + 2 * b - 2 * k + 3) / (factorial(2 * b + 1) * factorial(2 * a - 2 * k + 2)) * zeta(
            2 * a + 2 * b - 2 * k + 4) / s ** (2 * a + 2 * b - 2 * k + 4)

    return KroneckerDelta(k, a + b + 2) + KroneckerDelta(k, 0) * KroneckerDelta(k, 0) * zeta(2 * a + 2) * zeta(
        2 * b + 2) \
           / s ** (2 * a + 2 * b + 4) + p1 + p2
            ...
            if n > 1:
                ...
                for hh in range(0, g+1):
                    ...
                    for part in parts:   #total1
                        ...
                        r321 += f(a, b, c, d, hh, 1 + len(kpart1), [aa] + kpart1) \
                               * f(a, b, c, d, g - hh, 1 + len(kpart2), [bb] + kpart2)
                ...
                for hh in range(0, g+1):
                    ...
                    for part in parts:
                        kpart1 = [k[i - 1] for i in part[0]]
                        kpart2 = [k[i - 1] for i in part[1]]
                        r322 += f(a, b, c, d, hh, 1 + len(kpart2), [aa] + [k[i-1] for i in kpart2]) \
                               * f(a, b, c, d, g - hh, 1 + len(kpart1), [bb] + [k[i-1] for i in kpart1])
                r32 += r322
